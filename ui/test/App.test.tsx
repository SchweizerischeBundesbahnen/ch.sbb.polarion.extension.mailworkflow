...
const aboutRoutes = () => [
  {
    method: 'GET',
    match: /\/version$/,
    json: { bundleName: 'Mail Workflow', bundleVendor: 'SBB', bundleVersion: '1.0.0' },
  },
...
afterEach(() => {
  ...
  document.cookie = 'mailworkflow-dev-scope=; path=/; max-age=0';
});
...
describe('App router', () => {
  it('renders the Landing stub with every feature linked when no feature is selected', async () => {
    ...
    expect(links).toEqual(['?feature=about']);
  });
  ...
  it('renders the About page for ?feature=about', async () => {
    ...
    expect(document.body.textContent).toContain('Mail Workflow');
...
describe('accessibility', () => {
  ...
  it('has no WCAG A/AA violations on the dev Landing with a load error', async () => {
    installFetchMock([
      { method: 'GET', match: /\/polarion\/rest\/v1\/projects/, respond: () => jsonResponse({}, 401) },
    ]);
    window.history.replaceState({}, '', '?');
